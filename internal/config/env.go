...
func AgentEnv(cfg AgentEnvConfig) map[string]string {
	...
	//
	// Resolution: running daemon state/config first, then process env fallback.
	// Process env propagation ensures agent sessions inherit the port even when
	// TownRoot is not set (e.g., AgentEnvSimple callers).
	if cfg.TownRoot != "" {
		if port := ResolveDoltPort(cfg.TownRoot); port > 0 {
			portStr := strconv.Itoa(port)
			env["GT_DOLT_PORT"] = portStr
			env["BEADS_DOLT_PORT"] = portStr
			env["BEADS_DOLT_SERVER_PORT"] = portStr
		}
	}
...
// by parsing only the small state/config fragments needed for client env.
func ResolveDoltPort(townRoot string) int {
	// 1. Prefer the running daemon state: spawned clients must connect to the
	// actual server, even when older config/env data still says 3307.
	if port := readRunningDoltStatePort(townRoot); port > 0 {
		return port
	}
...
func readRunningDoltStatePort(townRoot string) int {
	if townRoot == "" {
		return 0
	}
	data, err := os.ReadFile(filepath.Join(townRoot, "daemon", "dolt-state.json"))
	if err != nil {
		return 0
	}
	var state struct {
