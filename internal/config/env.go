...
func AgentEnv(cfg AgentEnvConfig) map[string]string {
	...
	// Explicit GT_DOLT_* values are authoritative; stale Beads env is mirrored
	// only when no GT value is present.
	if cfg.TownRoot != "" {
	...
	// Propagate Dolt server host. GT_DOLT_HOST is authoritative over stale
	// BEADS_DOLT_SERVER_HOST for the same reason GT_DOLT_PORT is authoritative.
	if v := os.Getenv("GT_DOLT_HOST"); v != "" {
		env["GT_DOLT_HOST"] = v
		env["BEADS_DOLT_SERVER_HOST"] = v
	} else if _, ok := env["BEADS_DOLT_SERVER_HOST"]; !ok {
