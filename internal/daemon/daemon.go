...
func (d *Daemon) isRigOperational(rigName string) (bool, string) {
	...
	rigBeadID := fmt.Sprintf("%s-rig-%s", config.GetRigPrefix(d.config.TownRoot, rigName), rigName)
	if issue, err := rig.ShowIdentityBead(d.config.TownRoot, rigName); err == nil {
