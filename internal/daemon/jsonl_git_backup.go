...
func (d *Daemon) commitAndPushJsonlBackup(gitRepo string, databases []string, counts map[string]int, failed []string) error {
	// Stage only successfully exported database paths.
	// Never stage the whole repo here: a misconfigured backup path must not turn
	// a JSONL export into a repo-wide deletion commit.
	stagePaths := jsonlBackupStagePaths(databases, counts)
...
func jsonlBackupStagePaths(databases []string, counts map[string]int) []string {
	seen := make(map[string]bool, len(databases))
	paths := make([]string, 0, len(databases))
	for _, db := range databases {
		if _, ok := counts[db]; !ok || seen[db] {
			continue
		}
		seen[db] = true
