...
const (
	...
	// should surface a warning. Raw open wisp counts remain diagnostic only.
...
func countAlertableWisps(ctx context.Context, runner sqlRunner, dbName string, cutoff time.Time) (moleculeSteps, staleWisps int, err error) {
	...
	if err := runner.QueryRowContext(ctx, moleculeStepQuery).Scan(&moleculeSteps); err != nil {
		return 0, 0, fmt.Errorf("count molecule step candidates: %w", err)
	}

	// Count stale candidates with the same eligibility as Reap(). Closed-molecule
	// steps are counted separately above and excluded here so counts stay disjoint.
	...
	staleQuery := fmt.Sprintf("SELECT COUNT(*) FROM wisps w %s %s WHERE %s", parentJoin, moleculeStepExcludeJoin, whereClause)
	if err := runner.QueryRowContext(ctx, staleQuery, cutoff).Scan(&staleWisps); err != nil {
		return 0, 0, fmt.Errorf("count reap candidates: %w", err)
	}

	return moleculeSteps, staleWisps, nil
}
...
func Scan(db *sql.DB, dbName string, maxAge, purgeAge, mailDeleteAge, staleIssueAge time.Duration) (*ScanResult, error) {
	...
	now := time.Now().UTC()
	moleculeSteps, staleWisps, err := countAlertableWisps(ctx, db, dbName, now.Add(-maxAge))
	if err != nil {
		return nil, err
	}
	result.MoleculeStepCandidates = moleculeSteps
	result.ReapCandidates = staleWisps
	result.AlertableWisps = moleculeSteps + staleWisps
...
func Reap(db *sql.DB, dbName string, maxAge time.Duration, dryRun bool) (*ReapResult, error) {
	...
	if dryRun {
		moleculeSteps, staleWisps, err := countAlertableWisps(ctx, db, dbName, cutoff)
		if err != nil {
			return nil, err
		}
		result.MoleculeStepsClosed = moleculeSteps
		result.Reaped = staleWisps
		result.AlertableRemain = moleculeSteps + staleWisps
		openQuery := "SELECT COUNT(*) FROM wisps WHERE status IN ('open', 'hooked', 'in_progress')"
	...
	moleculeStepsRemain, staleWispsRemain, err := countAlertableWisps(ctx, conn, dbName, cutoff)
	if err != nil {
		return result, fmt.Errorf("count alertable remain: %w", err)
	}
	result.AlertableRemain = moleculeStepsRemain + staleWispsRemain
