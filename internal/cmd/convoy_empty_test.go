...
case "$pos0" in
  list)
    echo '[{"id":"hq-blocks-type","title":"Blocks type convoy"}]'
    exit 0
    ;;
  sql)
    echo '[{"depends_on_id":"gt-ready1"},{"depends_on_id":"gt-blocked1"}]'
    exit 0
    ;;
  dep)
    echo '[{"id":"gt-ready1","title":"Ready issue","status":"open","issue_type":"task","assignee":"","dependency_type":"tracks"},{"id":"gt-blocked1","title":"Blocked issue","status":"open","issue_type":"task","assignee":"","dependency_type":"tracks"}]'
    exit 0
    ;;
  show)
    case "$*" in
      *gt-ready1*gt-blocked1*|*gt-blocked1*gt-ready1*)
        echo '[{"id":"gt-ready1","title":"Ready issue","status":"open","issue_type":"task","assignee":"","blocked_by":[],"blocked_by_count":0,"dependencies":[]},{"id":"gt-blocked1","title":"Blocked issue","status":"open","issue_type":"task","assignee":"","blocked_by":[],"blocked_by_count":0,"dependencies":[{"id":"gt-ready1","status":"open","type":"blocks","issue_type":"task"}]}]'
        ;;
      *gt-ready1*)
        echo '[{"id":"gt-ready1","title":"Ready issue","status":"open","issue_type":"task","assignee":"","blocked_by":[],"blocked_by_count":0,"dependencies":[]}]'
        ;;
      *gt-blocked1*)
        echo '[{"id":"gt-blocked1","title":"Blocked issue","status":"open","issue_type":"task","assignee":"","blocked_by":[],"blocked_by_count":0,"dependencies":[{"id":"gt-ready1","status":"open","type":"blocks","issue_type":"task"}]}]'
        ;;
      *)
        echo '[{"id":"hq-blocks-type","title":"Blocks type convoy","status":"open","issue_type":"convoy"}]'
        ;;
    esac
    exit 0
    ;;
...
`
	...
	s := stranded[0]
	if s.TrackedCount != 2 {
		t.Fatalf("TrackedCount = %d, want 2", s.TrackedCount)
	}
	if s.ReadyCount != 1 {
		t.Fatalf("ReadyCount = %d, want 1", s.ReadyCount)
	}
	if len(s.ReadyIssues) != 1 || s.ReadyIssues[0] != "gt-ready1" {
		t.Fatalf("ReadyIssues = %v, want [gt-ready1]", s.ReadyIssues)
	}
}
