...
func TestRunDoneRejectsMayorRigBeforeAutosave(t *testing.T) {
	...
	mayorRig := filepath.Join(townRoot, "mayor", "rig")
	initDoneGuardGitRepo(t, mayorRig)
	dirtyFile := filepath.Join(mayorRig, "unrelated.txt")
	...
	if err := os.Chdir(mayorRig); err != nil {
		t.Fatalf("chdir mayor rig: %v", err)
	}
	...
	status := doneGuardGitOutput(t, mayorRig, "status", "--short")
	if !strings.Contains(status, "?? unrelated.txt") {
		t.Fatalf("mayor rig status = %q, want dirty file left uncommitted", status)
	}
}
