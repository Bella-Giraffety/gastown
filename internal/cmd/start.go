...
func cleanupPolecats(townRoot string) {
	...
	for _, r := range rigs {
		...
		for _, p := range polecats {
			...
			expectation := polecat.RemoveExpectation{Validate: true, Branch: p.Branch, Issue: p.Issue}
			if err := polecatMgr.RemoveWithExpectation(p.Name, true, shutdownNuclear, false, expectation); err != nil {
