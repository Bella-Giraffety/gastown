...
type deaconLifecycle interface {
	Start(agentOverride string) error
	Stop() error
}

var newDeaconLifecycle = func() (deaconLifecycle, error) {
	townRoot, err := workspace.FindFromCwdOrError()
	if err != nil {
		return nil, fmt.Errorf("not in a Gas Town workspace: %w", err)
	}
	return deacon.NewManager(townRoot), nil
}

var deaconSessionExists = func(sessionName string) (bool, error) {
	return tmux.NewTmux().HasSession(sessionName)
}

var attachDeaconSession = attachToTmuxSession
...
func runDeaconStart(cmd *cobra.Command, args []string) error {
	manager, err := newDeaconLifecycle()
	if err != nil {
		return err
	}

	fmt.Println("Starting Deacon session...")
	if err := manager.Start(deaconAgentOverride); err != nil {
...
func runDeaconStop(cmd *cobra.Command, args []string) error {
	...
	if err := manager.Stop(); err != nil {
		if errors.Is(err, deacon.ErrNotRunning) {
...
func runDeaconAttach(cmd *cobra.Command, args []string) error {
	...
	running, err := deaconSessionExists(sessionName)
	if err != nil {
	...
	if !running {
		...
		if err := manager.Start(deaconAgentOverride); err != nil && !errors.Is(err, deacon.ErrAlreadyRunning) {
			return err
		}
	}
	...
	return attachDeaconSession(sessionName)
}
...
func runDeaconRestart(cmd *cobra.Command, args []string) error {
	...
	if err := manager.Stop(); err != nil && !errors.Is(err, deacon.ErrNotRunning) {
