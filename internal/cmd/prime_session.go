...
func detectSessionState(ctx RoleContext) SessionState {
	...
	// Check for hooked work (autonomous state). The work row's status+assignee is
	// authoritative; hook_bead is retained only as a legacy compatibility hint.
	...
	if agentID != "" {
		...
		// Legacy compatibility: agent bead's hook_bead field, if still populated.
		...
		// Fallback: query authoritative work assignment across issues and wisps.
		hookedBeads, err := listAssignedWork(b, agentID, beads.StatusHooked, "in_progress")
		...
		if !isTownLevelRole(agentID) && ctx.TownRoot != "" {
			...
			if townWork, err := listAssignedWork(townB, agentID, beads.StatusHooked, "in_progress"); err == nil && len(townWork) > 0 {
				state.State = "autonomous"
