...
var healthDatabaseCount = health.DatabaseCount
...
func runHealth(cmd *cobra.Command, args []string) error {
	...
	if report.Server.Running {
		visibleDatabases = listVisibleHealthDatabases(report.Server.Port)
		report.Databases = checkDatabaseHealth(report.Server.Port, visibleDatabases)
	}

	// 3. Pollution scan
	if report.Server.Running {
		report.Pollution = checkPollution(report.Server.Port, visibleDatabases)
	}
...
func listVisibleHealthDatabases(port int) []string {
	_, databases, err := healthDatabaseCount("127.0.0.1", port)
	if err != nil {
		return nil
	}
...
func checkDatabaseHealth(port int, databaseNames []string) []DatabaseHealth {
...
func checkPollution(port int, databaseNames []string) []PollutionRecord {
