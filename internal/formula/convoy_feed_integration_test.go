...
func TestMolOrphanScanScopeDefaultsToTown(t *testing.T) {
	path := filepath.Join("formulas", "mol-orphan-scan.formula.toml")
	data, err := os.ReadFile(path)
	if err != nil {
		t.Skipf("Formula not found: %v", err)
	}
	...
	if scope.Required {
		t.Error("scope should not be required when it has a default")
	}
	if scope.Default != "town" {
		t.Errorf("scope default = %q, want %q", scope.Default, "town")
	}
}
