...
func TestLookupPullRequestRecordedURLRejectsHeadMismatch(t *testing.T) {
	installFakeGH(t, `#!/bin/sh
if [ "$1" = "pr" ] && [ "$2" = "view" ] && [ "$3" = "https://github.com/upstream/repo/pull/42" ]; then
  printf '%s\n' '{"number":42,"url":"https://github.com/upstream/repo/pull/42","state":"OPEN","mergedAt":"","headRefName":"fix/head-moved","headRefOid":"actual","headRepository":{"nameWithOwner":"fork/repo"},"headRepositoryOwner":{"login":"fork"},"baseRepository":{"nameWithOwner":"upstream/repo"}}'
  exit 0
fi
printf 'unexpected gh args: %s\n' "$*" >&2
exit 1
`)
	...
	_, err := g.LookupPullRequest(PullRequestRef{URL: "https://github.com/upstream/repo/pull/42", Branch: "fix/head-moved", HeadSHA: "expected"})
	if err == nil || !strings.Contains(err.Error(), "does not match submitted head") {
		t.Fatalf("LookupPullRequest err = %v, want submitted head mismatch", err)
	}
}
...
`)
	...
	for _, want := range []string{
		...
		"pr merge https://github.com/upstream/repo/pull/42 --squash --delete-branch --match-head-commit abc123 --repo upstream/repo",
	} {
