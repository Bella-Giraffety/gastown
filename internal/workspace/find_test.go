...
func TestFindFromCwdOrErrorPrefersGTTownRoot(t *testing.T) {
	townRoot := realPath(t, t.TempDir())
	if err := os.MkdirAll(filepath.Join(townRoot, "mayor"), 0755); err != nil {
		t.Fatalf("mkdir town mayor: %v", err)
	}
	if err := os.WriteFile(filepath.Join(townRoot, "mayor", "town.json"), []byte(`{"name":"outer"}`), 0644); err != nil {
		t.Fatalf("write town.json: %v", err)
	}

	nested := filepath.Join(townRoot, "other-town")
	if err := os.MkdirAll(filepath.Join(nested, "mayor"), 0755); err != nil {
		t.Fatalf("mkdir nested mayor: %v", err)
	}
	if err := os.WriteFile(filepath.Join(nested, "mayor", "town.json"), []byte(`{"name":"inner"}`), 0644); err != nil {
	...
	if got != townRoot {
		t.Errorf("FindFromCwdOrError() = %q, want %q", got, townRoot)
	}
}
