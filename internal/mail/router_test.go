...
func TestAddressToSessionIDs(t *testing.T) {
	...
	tests := []struct {
		...
		// Dogs are town-level under Deacon but route to the dog session.
...
func TestAddressToAgentBeadID(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "dog path with extra segment",
			address:  "deacon/dogs/alpha/extra",
			expected: "",
		},
		{
			name:     "reserved deacon subpath",
			address:  "deacon/foo",
			expected: "",
		},
		{
			name:     "reserved mayor subpath",
			address:  "mayor/foo",
			expected: "",
		},
	}
