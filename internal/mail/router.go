...
//
// Explicit GT_TOWN_ROOT/GT_ROOT overrides win when they describe the caller's
// town, or when startDir is outside any workspace.
func detectTownRoot(startDir string) string {
	townRoot, err := workspace.FindFromStartDirOrEnv(startDir)
	if err == nil && townRoot != "" {
		return townRoot
	}
	return ""
