...
func (m *Manager) RemoveWithOptions(name string, force, nuclear, selfNuke bool) (retErr error) {
	...
	// Best-effort: Push a polecat work branch to remote before non-nuclear removal.
	// Nuclear cleanup callers run their own target-aware preservation gate before
	// reaching this point; repeating this target-unaware push can resurrect stale
	// source branches after they have already landed.
	if !nuclear {
		polecatGit := git.NewGit(clonePath)
...
func (m *Manager) workstateInputForPolecat(name string, state State, issue string) WorkstateInput {
	...
	if input.MQCheckRequired && input.HasSubmittableWork && !workTerminal && !input.MQNotRequired {
