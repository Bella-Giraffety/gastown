...
`, logPath, townBeadsDir)
	...
	if !strings.Contains(logOutput, "env="+townBeadsDir+" args=create") {
		t.Fatalf("manager create did not use town BEADS_DIR; log:\n%s", logOutput)
	}
	if !strings.Contains(logOutput, "env="+townBeadsDir+" args=show") || !strings.Contains(logOutput, "env="+townBeadsDir+" args=update") {
