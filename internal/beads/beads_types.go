...
func bdPinnedEnv(beadsDir string) []string {
	return EnvWithValue(EnvForBeadsDir(os.Environ(), beadsDir), "BD_DOLT_AUTO_COMMIT", "on")
}
...
func ensureDatabaseInitialized(beadsDir string) error {
	...
	migrateEnv := bdPinnedEnv(beadsDir)
