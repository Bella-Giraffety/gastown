...
Use the native guide for platform-specific install commands, PATH setup, workspace creation, and first-rig setup: [docs/INSTALLING.md](docs/INSTALLING.md). In short: install `gt`, configure your Git identity, run `gt install ~/gt --shell --git`, start services with `gt up`, verify with `gt doctor --fix`, add a rig, and attach the Mayor.

### Docker Compose setup

Use the Docker guide for Compose setup, storage, security, and lifecycle details: [docs/docker.md](docs/docker.md). The Docker path needs Docker with Compose v2 and a dedicated empty host directory for `${FOLDER}` on first start; do not point it at a native HQ because Compose overlays `/gt/.dolt-data` with a Docker-managed volume.
