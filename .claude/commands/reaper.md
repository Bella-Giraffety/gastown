...
- `open_wisps`: total open wisp inventory for context
- `alertable_wisps`: cleanup backlog that can trigger escalation
- `anomalies`: array of detected problems

If `alertable_wisps` exceeds 800 across all databases, note for escalation.
Do not escalate on `open_wisps` alone; it includes healthy active work.
