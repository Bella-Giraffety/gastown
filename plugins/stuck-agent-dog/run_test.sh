...
case "${1:-}" in
  ...
  session)
    if [ "${2:-}" = "health" ]; then
      ...
      printf '%s --max-inactivity %s\n' "$session" "${6:-}" >> "$TEST_STATE/health_calls.log"
      healthy=false
      zombie=false
      case "$status" in
...
setup_case() {
  ...
  mkdir -p "$TEST_STATE/health" "$TEST_STATE/sessions" "$TEST_STATE/nohook" "$bin_dir"
  mkdir -p "$GT_TOWN_ROOT/mayor" "$GT_TOWN_ROOT/gastown/polecats" "$GT_TOWN_ROOT/deacon"
  printf '{"rigs":{"gastown":{"beads":{"prefix":"gt"}}}}\n' > "$GT_TOWN_ROOT/mayor/rigs.json"
  : > "$TEST_STATE/mail.log"
  : > "$TEST_STATE/kill.log"
  : > "$TEST_STATE/escalate.log"
